...
import { Search, Filter, Calendar as CalendarIcon, Package, Cpu, ArrowRight, Layers } from 'lucide-react';
...
export const ScheduleView: React.FC<ScheduleViewProps> = ({ entries, onReset, globalSearch = '' }) => {
  ...
  const machineGroups = useMemo(() => {
    ...
    entries.forEach(e => {
        ...
        if (groups[e.machine][e.productCode][e.material]) {
            groups[e.machine][e.productCode][e.material].totalQuantity += qty;
        } else {
  ...
  return (
    <div className="w-full max-w-6xl mx-auto p-4 md:p-6 space-y-6">
                        ...
                        <tr key={i} className="group hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-colors">
                           ...
                           <td className="px-4 py-5">
                              <div className="flex items-center gap-2">
                                 ...
                                 <span className="font-black text-slate-900 dark:text-white uppercase text-xs sm:text-sm">Máq {res.machine}</span>
                              </div>
                           </td>
                           <td className="px-4 py-5">
      ...
      {/* Date Selector */}
      <div className="overflow-x-auto pb-4 -mx-4 px-4 no-scrollbar scroll-smooth flex gap-3">
              ...
              <div className="mt-auto bg-slate-50 dark:bg-slate-800/40 px-6 py-4 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between">
                  ...
                  <div className="flex items-baseline gap-1">
                    <span className="text-sm font-black text-slate-900 dark:text-white">
                        {numberFormatter.format(group.products.reduce((acc, p) => acc + p.materials.reduce((ma, item) => ma + item.totalQuantity, 0), 0))}
                    </span>
